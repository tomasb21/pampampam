...
protected:
    ...
    GLMNETPP_STRONG_INLINE auto& penalty_matrix() { return p_; }
...
private:
    ...
    mat_t p_;               // matrix of penalty values for each coefficient with intercept at row 0?
...
protected:
    ...
    void update_beta(index_t k, value_t gk, value_t l1_regul, value_t l2_regul) {
        const auto& cl = this->endpts();
        base_t::update_beta(
                beta(k), gk, xv_ic_(k), penalty_matrix(k),
                cl(0,k), cl(1,k), l1_regul, l2_regul);
    }
    ...
    state_t setup_wls(size_t ic) {
        ...
        new (&p_ic_ ) Eigen::Map<vec_t>      (ppp.col(ic).data(), ppp.rows());
